...
public class phase4 {
	...
	static ArrayList<Integer> truepartition;
	static ArrayList<Integer> yhat;
	...
	static double FN = 0;
	static double N = 0;
	static double Joutput = 0;
	static double Randoutput = 0;
	static double FMoutput = 0;
	static double bestJ = 0;
	static double bestRand = 0;
	static double bestFM = 0;
	...
	public static void main(String[] args) throws IOException {
		...
		PrintStream o = null;
		try {
			o = new PrintStream(new File("./phase1/phase4_" + fn));
		...
		for (int i = 0; i < numRuns; i++) {
			...
			do {
				...
				//Calculate the external validations.
				PairwiseMeasureCalc();
		...
		System.out.println("\nBest Jaccad: " + bestJ + "      Best Rand: " + bestRand + "      Best FM: " + bestFM);
	...
	{
		int linecounter = 0;
		truepartition = new ArrayList<Integer>();
		...
		try (BufferedReader br = new BufferedReader(new FileReader("./phase1/phase4_data_sets/" + fn))) {
			//numbers = new ArrayList<>();
		...
		public static void PairwiseMeasureCalc()
		{
			//Calculate the pairwise measures.
			for (int i=0; i<Points.size(); i++)
			{
				for (int j=0; j<Points.size(); j++)
				{
					...
					{
						//Calculate TP
						if ((yhat.get(i) == yhat.get(j))&&((yhat.get(i) == truepartition.get(i))&&(yhat.get(j) == truepartition.get(j))))
							TP += 1;
						else if ((yhat.get(i) != yhat.get(j))&&(truepartition.get(i)!=truepartition.get(j)))
							TN += 1;
						else if ((yhat.get(i) == yhat.get(j)) && (truepartition.get(i) != truepartition.get(j)))
							FP += 1;
						else if ((yhat.get(i) != yhat.get(j)) && (truepartition.get(i) == truepartition.get(j)))
		...
		public static void FM() {
			//Calc Fowlkes-Mallows Measure 
			double prec = TP / (TP + FP);
			double recall = TP / (TP + FN);
			FMoutput = Math.sqrt(prec * recall);
		}
	}
